...
import provider
import pointnet_cls_rot_partseg as model
...
parser.add_argument('--model_path', type=str, default='/home/nx38/pointnet/log_rotation_shapenet_all_directions/rot_angles_32_batch_4_opt_adam_lr_0.001_trans_loss_False_input_trans_False_feature_trans_False/model.ckpt')
#rot_angles_6_batch_32_opt_adam_lr_0.001_trans_loss_False_input_trans_False_feature_trans_False
...
def placeholder_inputs_pretrain():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(FLAGS.gpu)):
            ...
            labels_pred, seg_pred, end_points = model.get_model(pointclouds_ph, input_label_ph, \
                                                                ...
                                                                num_point=point_num, weight_decay=FLAGS.wd, use_input_trans=False, use_feature_trans = False)
            ...
            for v in tf.trainable_variables():
                if v.name.startswith('pointnet_part_seg'):
        ...
        for epoch in range(TRAINING_EPOCHES):
            ...
            if (epoch + 1) % 10 == 0:
                cp_filename = param_saver.save(sess, os.path.join(MODEL_STORAGE_PATH, 'epoch_' + str(epoch + 1) + '.ckpt'))
