...
def get_loss(pred, label, end_points, angle_weight=1, reg_weight=0.001, use_angle_loss = False, use_trans_loss=False):
	""" pred: B*NUM_CLASSES,
		label: B, """
	# loss = tf.losses.softmax_cross_entropy(onehot_labels=labels, logits=pred, label_smoothing=0.2)
	num_angles = pred.shape[1]
	loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=label)
	classify_loss = tf.reduce_mean(loss)
	angle_diffs = get_rotated_angle_diff(num_angles, tf.argmax(pred, 1), label)
	angle_loss = tf.reduce_sum(angle_diffs)
	...
	loss = classify_loss
	if use_angle_loss:
		loss += angle_weight * angle_loss
	if use_trans_loss:
		loss += mat_diff_loss * reg_weight
