...
def files_in_subdirs(top_dir, file_ending):
    for path in glob.glob(top_dir+'/'+file_ending):
        yield path
    # regex = re.compile(file_ending)
...
def load_all_shapenet_data(top_dir, n_threads=20, file_ending='.ply', verbose=False):
    labels_lst = list(snc_category_to_synth_id().keys())
    all_pc = None
    for label in tqdm.tqdm(labels_lst, desc='loading data'):
        syn_id = snc_category_to_synth_id()[label]
        class_dir = os.path.join(top_dir, syn_id)
        if not all_pc:
            all_pc = load_all_point_clouds_under_folder(class_dir, n_threads=8, file_ending='*.ply', verbose=False)
        else:
            pc = load_all_point_clouds_under_folder(class_dir, n_threads=8, file_ending='*.ply', verbose=False)
            all_pc = all_pc.merge(pc)
    return all_pc
