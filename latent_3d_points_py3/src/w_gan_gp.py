...
class W_GAN_GP(GAN):
    ...
    def __init__(self, name, discriminator, generator, gen_kwargs={}, disc_kwargs={}, gan_kwargs = {}, graph=None, **kwargs):
        ...
        with tf.variable_scope(name):
            ...
            self.opt_d = self.optimizer(init_lr, beta, self.loss_d, d_params)
            self.opt_g = self.optimizer(init_lr, beta, self.loss_g, g_params)
    ...
    def _single_epoch_train(self, train_data, epoch, batch_size, noise_params, discriminator_boost=5, writer=None):
        '''
