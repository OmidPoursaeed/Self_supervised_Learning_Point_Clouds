...
class GAN(Neural_Net):
    ...
    def optimizer(self, learning_rate, beta, loss, var_list, batch=None):
        initial_learning_rate = learning_rate
        optimizer = tf.compat.v1.train.AdamOptimizer(initial_learning_rate, beta1=beta).minimize(loss, var_list=var_list, global_step=batch)
        return optimizer
